...
export function latlngToDMS(location) {
    ...
    const latDeg = Math.trunc(latitude);
    const latMin = Math.trunc((latitude - latDeg) * 12) * 5;

    const longDeg = Math.trunc(longitude);
    const longMin = Math.trunc((longitude - longDeg) * 12) * 5;
